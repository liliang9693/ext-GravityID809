...
class ID809():
    ...
    def connected(self):
        header = self.__pack(self.CMD_TYPE, self.CMD_DEVICE_INFO, 0, 0)
        self.__send_packet(header)
        if self.ISIIC == True:
            time.sleep(0.05)
        ret = self.__response_payload()
    ...
    def __read_prefix(self, header):
        ...
        # header[8] = RET[0]
        # header[9] = RET[1]
        header[4] = self.__read_n(1)[0]
        header[5] = self.__read_n(1)[0]
        header[6] = self.__read_n(1)[0]
        header[7] = self.__read_n(1)[0]
        header[8] = self.__read_n(1)[0]
        header[9] = self.__read_n(1)[0]
        return ret
    ...
    def __read_to_addr(self, lens):
        rslt = self._i2c.readfrom(self.i2c_addr, lens)
        if rslt is None:
            print(f"ID809 rslt:{rslt}")
            return ([0]*len)
        else:
            return rslt
